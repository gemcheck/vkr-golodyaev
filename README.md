...
Разработано: Голодяевым Максимом Андреевичом
...
5. **Запустите проект**
   ...
   При первом запуске необходимо обязательно запустить проект, чтобы создались все необходимые папки и файлы.
...
Для корректного стягивания **Веб-платформы** пользуйтесь файлом `deploy.sh`. Линукс имеет свойсто перезатирать прафа на файлы. Также при изменении названия БД **обязательно** настройте права.

> Любые изменения на сервере будут затираться.
...
* **tests/web/**:
    ...
    * `flask_session/` - управление сессиями.
    * `.env` - файл переменных окружения.
    * `app.wsgi` - конфигурационный файл WSGI.
...
Разработано исключительно в учебных и научно-исследовательских целях в рамках подготовки ВКР, итоги работы запрещается использваоть в коммерческих целях. 
